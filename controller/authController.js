...
const registerUser = async (req,res) => {
    try {
        ...
        // res.json({ data: {...req.body , password: encPass} } )
    } catch (err) {
...
const loginUser = async (req,res) => {
    try {
        ...
        if(!extUser) return res.status(StatusCodes.NOT_FOUND).json({ status: false , msg: "Requested user does not exist"})

            // validate if password are same or not
            let passVal = await bcrypt.compare(password,extUser.password)
            if(!passVal) return res.status(StatusCodes.UNAUTHORIZED).json({ status: false, msg: "Password are not matched"})
                ...
                // final response
        res.status(StatusCodes.OK).json({ status: false , msg: "Login Success " ,  token })
    } catch (err) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({status: false, msg: err.message });
    }
}
...
const verifyUserToken = async (req,res) => {
    try {
        ...
        if(!extUser) 
            return res.status(StatusCodes.NOT_FOUND).json({status: false, msg:`requested user does not found`})
